...
describe("buyToken tests", function () {
    ...
    it("check mint tokens for buyer", async () => {
        ...
        expect(await Token.connect(user2).balanceOf(user2.address)).to.equal(BigNumber.from("100000000000000000000"))
    })
    it("check token calculation is proper", async () => {
        await Token.connect(user2).buyToken({
            value: ethers.utils.parseEther("0.5"),
        })
        expect(await Token.connect(user2).balanceOf(user2.address)).to.equal(BigNumber.from("5000000000000000000000"))
    })
})
