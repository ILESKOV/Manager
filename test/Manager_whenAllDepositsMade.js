...
beforeEach(async () => {
  ...
  await Token.transfer(user11.address, 5500);
  ...
  await Token.connect(user11).approve(Manager.address, 5000);
...
describe("test of not allowing more than 10 depositors", function () {
  it("check 11's depositor rejected", async () => {
    await expect(Manager.connect(user11).deposit()).to.be.revertedWith(
