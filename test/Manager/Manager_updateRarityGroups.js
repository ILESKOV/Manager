...
beforeEach(async () => {
    ...
    await Manager.updateRarityGroups(100, 5, 3, 1, 91)
...
describe("updateRarityGroups tests", function () {
    describe("negative tests", function () {
        it("should revert if rarity groups setted uncorrectly", async () => {
            await expect(Manager.updateRarityGroups(100, 33, 54, 12, 99)).to.be.revertedWith(
                "Sum of elements should equal depositorsLimit"
            )
        })
    })
    describe("positive tests", function () {
        it("should return new correct depositors limit", async () => {
            expect(await Manager.getDepositorsLimit()).to.equal(100)
        })
        it("should change ownership id for user1", async () => {
            expect((await Manager.getOwnershipRecord(user1.address))[0]).to.equal(BigNumber.from("1"))
        })
        it("should change ownership id for user3", async () => {
            expect((await Manager.getOwnershipRecord(user7.address))[0]).to.equal(BigNumber.from("7"))
        })
        it("check rarity setted properly for user1", async () => {
            expect((await Manager.getOwnershipRecord(user1.address))[1]).to.equal(5000)
        })
        it("check rarity setted properly for user2", async () => {
            expect((await Manager.getOwnershipRecord(user2.address))[1]).to.equal(5000)
        })
        it("check rarity setted properly for user3", async () => {
            expect((await Manager.getOwnershipRecord(user3.address))[1]).to.equal(5000)
        })
        it("check rarity setted properly for user4", async () => {
            expect((await Manager.getOwnershipRecord(user4.address))[1]).to.equal(5000)
        })
        it("check rarity setted properly for user5", async () => {
            expect((await Manager.getOwnershipRecord(user5.address))[1]).to.equal(5000)
        })
        it("check rarity setted properly for user6", async () => {
            expect((await Manager.getOwnershipRecord(user6.address))[1]).to.equal(3000)
        })
        it("check rarity setted properly for user7", async () => {
            expect((await Manager.getOwnershipRecord(user7.address))[1]).to.equal(3000)
        })
        it("check rarity setted properly for user8", async () => {
            expect((await Manager.getOwnershipRecord(user8.address))[1]).to.equal(3000)
        })
        it("check rarity setted properly for user9", async () => {
            expect((await Manager.getOwnershipRecord(user9.address))[1]).to.equal(1000)
        })
        it("check rarity setted properly for user10", async () => {
            ...
            expect((await Manager.getOwnershipRecord(user10.address))[1]).to.equal(91000)
        })
    })
})
