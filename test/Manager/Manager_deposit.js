...
describe("deposit tests", function () {
    describe("negative tests", function () {
        it("should revert if user not approved tokens for Manager contract", async () => {
            await expect(Manager.connect(user1).deposit()).to.be.revertedWith("ERC20: insufficient allowance")
        })
    ...
    describe("positive tests", function () {
        ...
        it("should check balance of user tokens change after deposit", async () => {
            ...
            await Manager.connect(user1).deposit()
            expect(await Token.connect(user1).balanceOf(user1.address)).to.equal(
                BigNumber.from("7000000000000000000000")
            )
        })
