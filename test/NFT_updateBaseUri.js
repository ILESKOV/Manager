...
describe("update metadata URI", function () {
  it("should revert if 10 NFTs wasn't minted", async () => {
    await expect(
      Manager.updateBaseURI(
        "ipfs://bafybeig5yw7u5xy3oynsyxaazkbtu66jxblwexh4aghb3r7dyozdh5upxi/"
      )
    ).to.be.revertedWith("Wait to all tokens to be minted");
  });
  it("should revert if caller not the owner", async () => {
    ...
    await expect(
      Manager.connect(user10).updateBaseURI(
        "ipfs://bafybeig5yw7u5xy3oynsyxaazkbtu66jxblwexh4aghb3r7dyozdh5upxi/"
      )
  ...
  it("should revert if URI was already updated by owner", async () => {
    ...
    await Manager.updateBaseURI(
      "ipfs://bafybeig5yw7u5xy3oynsyxaazkbtu66jxblwexh4aghb3r7dyozdh5upxi/"
    );
    await expect(
      Manager.updateBaseURI("ipfs://bafybeig5ywreverteffxb/")
    ).to.be.revertedWith("Base URI was already updated");
  });
