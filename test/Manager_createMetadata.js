...
describe("createMetadata tests", function () {
    ...
    it("check rarity setted properly for user1", async () => {
        expect((await Manager.getOwnershipRecord(user1.address))[1]).to.equal("40%")
    })
    it("check rarity setted properly for user2", async () => {
        expect((await Manager.getOwnershipRecord(user2.address))[1]).to.equal("40%")
    })
    it("check rarity setted properly for user3", async () => {
        expect((await Manager.getOwnershipRecord(user3.address))[1]).to.equal("40%")
    })
    it("check rarity setted properly for user4", async () => {
        expect((await Manager.getOwnershipRecord(user4.address))[1]).to.equal("40%")
    })
    it("check rarity setted properly for user5", async () => {
        expect((await Manager.getOwnershipRecord(user5.address))[1]).to.equal("30%")
    })
    it("check rarity setted properly for user6", async () => {
        expect((await Manager.getOwnershipRecord(user6.address))[1]).to.equal("30%")
    })
    it("check rarity setted properly for user7", async () => {
        expect((await Manager.getOwnershipRecord(user7.address))[1]).to.equal("30%")
    })
    it("check rarity setted properly for user8", async () => {
        expect((await Manager.getOwnershipRecord(user8.address))[1]).to.equal("20%")
    })
    it("check rarity setted properly for user9", async () => {
        expect((await Manager.getOwnershipRecord(user9.address))[1]).to.equal("20%")
    })
    it("check rarity setted properly for user10", async () => {
        await Manager.connect(user10).createNFT(
            "ipfs://bafybeid2iuyhqr4uxrtkyqw3x4n7ynhuteasbe4doywijmtb4qmk7bxsmm/10.png"
        )
        expect((await Manager.getOwnershipRecord(user10.address))[1]).to.equal("10%")
    })
